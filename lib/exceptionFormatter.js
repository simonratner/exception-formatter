...
(function() {
  ...
  parseException = (function() {
    ...
    return function(exception, options) {
      var basepath, basepathReplacement, endOfHeader, exceptionText, lines, ref, ref1, value;
      if (options == null) {
        options = {};
      }
      basepath = (ref = options.basepath) != null ? ref : process.cwd();
      basepathReplacement = (ref1 = options.basepathReplacement) != null ? ref1 : "." + path.sep;
      ...
      return lines.map(function(line) {
        ...
        parsed = null;
        if (indexOf.call(DIVIDERS, line) >= 0) {
          endOfHeader = true;
          type = DIVIDER;
        } else {
  ...
  formatExceptionLines = function(exception, options, lineFn) {
    var i, indent, j, line, lines;
    lines = parseException(exception, options);
    indent = function(s) {
      var ref;
      return (((ref = lines[lines.length - 1].parsed) != null ? ref.indent : void 0) || '') + s;
    };
    if (options.maxLines && lines.length > options.maxLines) {
      lines = lines.slice(0, +options.maxLines + 1 || 9e9);
      lines.push({
        line: indent('[truncated]', {
          type: DIVIDER
        })
      });
    }
    if (options.maxLines === 'auto') {
      for (i = j = lines.length - 1; j >= 0; i = j += -1) {
        line = lines[i];
        if (line.type === OUR_SOURCE) {
          lines = lines.slice(0, +i + 1 || 9e9);
          lines.push({
            line: indent('[truncated]', {
              type: DIVIDER
            })
  ...
  formatters = {
    html: (function() {
      ...
      return function(exception, options) {
        ...
        html = formatExceptionLines(exception, options, function(arg) {
          var line, ref, style, type;
          line = arg.line, type = arg.type;
          line = escapeHtml(line);
          style = options.inlineStyle ? "style='" + ((ref = INLINE_STYLES[type]) != null ? ref : '') + "'" : "class=" + type;
          switch (type) {
            case HEADER:
    ...
    ascii: function(exception, options) {
      ...
      return formatExceptionLines(exception, options, function(arg) {
        var line, type;
        line = arg.line, type = arg.type;
        switch (type) {
          case OUR_SOURCE:
    ...
    ansi: (function() {
      ...
      return function(exception, options) {
        ...
        return formatExceptionLines(exception, options, function(arg) {
          var line, parsed, ref, type;
          line = arg.line, type = arg.type, parsed = arg.parsed;
          switch (type) {
            case OUR_SOURCE:
              if ((ref = options.colors) != null ? ref : true) {
  ...
  module.exports = function(exception, options) {
    ...
    format = (ref = options.format) != null ? ref : 'ascii';
